...
Sub Globals
	...
	Dim lb As Label
	Dim mm As Typeface : mm = mm.LoadFromAssets("Warso.ttf")
End Sub
...
Sub Activity_Create(FirstTime As Boolean)
	...
	lb.Text = "	Install  ကိုနွိပ္ပါ။ ျပီးရင္ေအာက္ပါ Change Font1 ကိုနွိပ္ပါ Warso.txj (သို့) Warso.itz ကိုဖြင့္ၿပီး Apply လုပ္ေပးလိုက္ပါ မရခဲ့ရင္ Change Font2 ကိုနွိပ္ျပီး Myanmar Warso နာမည္နဲ့ Theme ကိုေရြးေပးလိုက္ပါ။ နဂိုမူလေဖာင့္ကိုျပန္ထားခ်င္ရင္ Change Font2 ကိုနွိပ္ျပီး Default ကိုျပန္ေရြးထားနိုင္ပါတယ္။"
	...
	tlb.Initialize("tlb")
	tlb.Text = "Vivo"
	tlb.Color = Colors.rgb(205, 220, 57)
	...
	Banner.Initialize2("Banner","ca-app-pub-4173348573252986/5398733758",Banner.SIZE_SMART_BANNER)
	Dim height As Int
	...
	Interstitial.Initialize("Interstitial","ca-app-pub-4173348573252986/6875466951")
	Interstitial.LoadAd
...
Sub b1_Click
	...
	If File.Exists(File.DirRootExternal & "/Vivo Myanmar Font","Warso.txj") = True Then File.Delete(File.DirRootExternal,"Vivo Myanmar Font/Warso.txj")
	File.Copy(File.DirAssets,"Warso.txj",File.DirRootExternal & "/Vivo Myanmar Font","Warso.txj")
	
	If File.Exists(File.DirRootExternal & "/Vivo Myanmar Font","Warso.itz") = True Then File.Delete(File.DirRootExternal,"Vivo Myanmar Font/Warso.itz")
	File.Copy(File.DirAssets,"Warso.itz",File.DirRootExternal & "/Vivo Myanmar Font","Warso.itz")
...
Sub b4_Click
	...
	If File.Exists(File.DirRootExternal & "/Download/i Theme/Font","Warso.txj") = True Then File.Delete(File.DirRootExternal,"Download/i Theme/Font/Warso.txj")
	File.Copy(File.DirAssets,"Warso.txj",File.DirRootExternal & "/Download/i Theme/Font","Warso.txj")
	
	If File.Exists(File.DirRootExternal & "/Download/i Theme/Font","Warso.itz") = True Then File.Delete(File.DirRootExternal,"Download/i Theme/Font/Warso.itz")
	File.Copy(File.DirAssets,"Warso.itz",File.DirRootExternal & "/Download/i Theme/Font","Warso.itz")
	Try
		Dim i As Intent
		i.Initialize("", "")
...
Sub SlideMenu_Click(Item As Object)
	...
	Select Item
		...
		Case 7 :
			...
			copy.clrText
			copy.setText("#MyanmarWarsoFont App! Beautiful Myanmar Zawgyi Font Style!	You can Use in Samung, Oppo,Vivo, Huawei (EMUI) and Xiaomi (MIUI) without Root Access!!!! Download Free at : http://www.myanmarandroidapp.com/search?q=Myanmar+Warso+Font")
